...
public class Query {
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("---------------------------------");
			// ------------------------------------
			// ( 2 ) collection.find({"age" : {"$gt" : 10}})) --> get documents
			// by query
			...
			for (int p = 0; p < comparison_operators.length; p++) {
				...
				System.out
						.println("( 2." + (p + 1) + " ) .find({\"age\" : {\"" + comparison_operators[p] + "\" : 20}})");
				System.out.println("results --> " + cursor.count());
			}
			System.out.println("---------------------------------");
			// ------------------------------------
			// ( 3 ) collection.find({"name" : /j/}, {"name" : 1, "age" :
			// 1,"_id":0}) --> get documents with some keys
			...
			System.out.println("---------------------------------");
			// -----------------------------------
			// ( 4 ) collection.find({"age" : {"$in" : [10,20,30]}})) --> get
			// documents by query
			...
			for (int p = 0; p < or_operators.length; p++) {
				...
				cursor = collection.find(query);
				System.out
						.println("( 4." + (p + 1) + " ) .find({\"age\" : {\"" + or_operators[p] + "\" : [10,20,30]}})");
				System.out.println("results --> " + cursor.count());
			}
			System.out.println("---------------------------------");
			// -----------------------------------
			// ( 5 ) collection.find({"$or" : [ {"name":"John" },{"age":20}]})
			...
			System.out.println("---------------------------------");
			// -----------------------------------
			// ( 6 ) collection.find({"friends" : ["John","Tim"]},{"name" : 1,
			// "freinds" : 1,"_id":0})) --> get documents by query
			...
			cursor = collection.find(query, keys);
			System.out.println(
					"( 6 ) .find({\"friends\" : {$all:[\"John\",\"Tim\"]}},{\"name\" : 1, \"freinds\" : 1,\"_id\":0})");
			System.out.println("results --> " + cursor.count());
			try {
				while (cursor.hasNext()) {
					System.out.println(cursor.next());
			...
			System.out.println("---------------------------------");
			// ------------------------------------
			// ( 7 ) collection.find({"address" :
			// {"country":"US","state":"NY","city":"Buffalo"}},{"name" : 1,
			// "address" : 1,"_id":0})) --> get documents by query
			query = new BasicDBObject("address",
					new BasicDBObject("country", "US").append("state", "NY").append("city", "Buffalo"));
			keys = new BasicDBObject("name", 1).append("address", 1).append("_id", 0);
			cursor = collection.find(query, keys);
			System.out.println(
					"( 7 ) .find({\"address\" : {\"country\":\"US\",\"state\":\"NY\",\"city\":\"Buffalo\"}},{\"name\" : 1, \"address\" : 1,\"_id\":0})");
			System.out.println("results --> " + cursor.count());
			try {
				while (cursor.hasNext()) {
					System.out.println(cursor.next());
			...
			System.out.println("---------------------------------");
			// ------------------------------------
			// ( 8 ) collection.find({"address.state" : "NY"},{"name" : 1,
			// "address" : 1,"_id":0})) --> get documents by query
